...
class ServiceManSupabaseUser extends BaseAuthUser {
  ServiceManSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> serviceManSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = ServiceManSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
