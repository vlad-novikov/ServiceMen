...
class ServiceMenSupabaseUser extends BaseAuthUser {
  ServiceMenSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> serviceMenSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = ServiceMenSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
